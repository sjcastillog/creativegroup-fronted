...
import Viewer from 'react-viewer';
    ...
    const TdYounetEdit = memo(((props)=>{
        const { minWidth, width, headCells, headCell, valueInfo, fixed, color, align, bolder, fontSize, idEl } = props
        const [ leftValue, setLeftValue ] = useState('0px');
    ...
    const TdYounetEdit2 = memo(((props)=>{
        const { valueInfo, minWidth, width, headCells, headCell, fixed, color, align, bolder, fontSize, idEl, handleRowNew } = props
        const [ leftValue, setLeftValue ] = useState('0px');
    ...
    const TdYounetEditCadena = memo(((props)=>{
        const { valueInfo, minWidth, width, headCells, headCell, fixed, color, align, bolder, fontSize, idEl, handleRowNew, onlyCadenas } = props
        const [ leftValue, setLeftValue ] = useState('0px');
        const [ infoCell, setInfoCell ] = useState('');
        ...
        const handleDataCell = (e, value)=>{
            if(value === ''){
                setInfoCell('');
                handleRowNew(idEl, value);
            }else{
                setInfoCell(value);
                handleRowNew(idEl, value)
            }
    ...
    const TdYounetEditLocal = memo(((props)=>{
        const { valueInfo, minWidth, width, headCells, headCell, fixed, color, align, bolder, fontSize, idEl, handleRowNew, localesFilter, isResetCadena, handleResetCadena, isSelectCadena } = props
        const [ leftValue, setLeftValue ] = useState('0px');
        const [ infoCell, setInfoCell ] = useState('');
        ...
        return(
            <Fragment>
            {
                ...
                >
                    <Autocomplete
                        id={idEl}
                        options={localesFilter}
                        getOptionLabel={option=>option.local}
                        size='small'
                        disabled={!isSelectCadena}
                        inputValue={infoCell || ''}
                        margin='dense'
                        style={{marginTop:-8}}
                        renderInput={params => <TextField {...params}  variant="outlined" margin='dense'/>}
                        onInputChange={handleDataCell} 
                    />
                </TdYounetStyledFixed2>
                :
                <TdYounetStyled2
    ...
    const Roow = memo((props)=>{
        const { row, index, headCells, HandleClickRow, handleEditRow, handleDeleteRow } = props;
        ...
        const handleEdit= ()=>{
            const objeto = {...row};
            objeto.EditRowNow = true;
            handleEditRow(row.idGeneral, objeto)
        };

        const handlePreDelete = ()=>{
            handleDeleteRow(row.idGeneral)
        };
            ...
            <TrYounetStyled key={`fila-${row.idGeneral}`}  selected={statusRow}>
                <TdYounetStyled minWidth={100} width={100}>
                    <Checkbox
                        ...
                        size='small'
                    />
                    <IconButton color='primary' onClick={handleEdit} size='small' style={{marginTop:-12}}>
                        <EditIcon/>
                    </IconButton>
                    <IconButton color='primary' onClick={handlePreDelete} size='small' style={{marginTop:-12}}>
                        <DeleteIcon/>
                    </IconButton>
                </TdYounetStyled>
                { HeadCellArr }
    ...
    const RowNew = ({ handleSaveRowNew, handleDeleteRowNew, headCells, index, row, arrCadenas, onlyCadenas })=>{
        const [ dataRow, setDataRow ] = useState({cadena:'', local:'', ciudad:'', direccion:'', provincia:''});
        const [ localesFilter, setLocalesFilter ] = useState([]);
        const [ isResetCadena, setResetCadena ] = useState(false);
        const [ isSelectCadena, setSelectCadena ] = useState(false);

        const handleRowNew = useCallback((idEl, value)=>{
            if(idEl === 'cadena'){
                if(value === null || value === ''){
                    setDataRow({...dataRow, cadena:'', local:'', ciudad:'', direccion:'', provincia:''});
                    setLocalesFilter([]);
                    setResetCadena(true);
                    setSelectCadena(false);
                }else{
                    const fil = arrCadenas.filter(el=> el.cadena.toString() === value.toString());
                    setLocalesFilter(fil);
                    setDataRow({...dataRow, cadena:value, local:'', ciudad:'', direccion:'', provincia:''});
                    setSelectCadena(true);
                }
                
            }else if(idEl === 'local'){
                if(value === null || value === ''){ 
                    setDataRow({...dataRow, local:'', provincia:'', ciudad:'', direccion:''})
                }else{

                    const elFil = localesFilter.filter(el=> el.local.toString() === value.toString())[0];
                    console.log(elFil);
                    setDataRow({...dataRow, local:value, ciudad:elFil.ciudad, direccion:elFil.direccion, provincia:elFil.provincia})
                }
            }else{
                setDataRow({...dataRow, [idEl]:value});
            }
        },[dataRow]);

        const handleResetCadena = useCallback(()=>{
            setResetCadena(false);
        },[isResetCadena]);
                    ...
                    <Fragment>
                        {
                            ...
                            <TdYounetEditCadena
                                ...
                                bolder={value.bolder}
                                idEl={value.id}
                                handleRowNew={handleRowNew}
                                onlyCadenas={onlyCadenas}
                            />: index2 === 1 ?
                            <TdYounetEditLocal
                                key={`${index}_${index2}_${row.idGeneral}_Td`}
                                width={value.width} 
                                ...
                                idEl={value.id}
                                handleRowNew={handleRowNew}
                                handleResetCadena={handleResetCadena}
                                isResetCadena={isResetCadena}
                                isSelectCadena={isSelectCadena}
        ...
        const handleSave = ()=>{
            const objeto = {...dataRow};
            objeto.ColNew = false;
            objeto.idGeneral = row.idGeneral;
            handleSaveRowNew(row.idGeneral, objeto);
        };

        return(
            <TrYounetStyled key={`fila-${row.idGeneral}`} >
                <TdYounetStyled2 minWidth={100} width={100} >
                    <IconButton color='primary' size='small' onClick={handleSave} style={{marginLeft:20}}>
                        <CheckIcon/>
                    </IconButton>
                    <IconButton color='primary' size='small' onClick={handleDeleteRowNew} >
                        <DeleteIcon/>
                    </IconButton>
                </TdYounetStyled2>
                { HeadCellArr }
    ...
    const RowEdit = ({ handleSaveRowNew, handleDeleteRowNew, headCells, index, row, arrCadenas, onlyCadenas })=>{
        const [ dataRow, setDataRow ] = useState({cadena:'', local:'', ciudad:'', direccion:'', provincia:''});
        const [ localesFilter, setLocalesFilter ] = useState([]);
        const [ isResetCadena, setResetCadena ] = useState(false);
        const [ isSelectCadena, setSelectCadena ] = useState(false);
        ...
        return(
            <TrYounetStyled key={`fila-${row.idGeneral}`} >
                <TdYounetStyled2 minWidth={100} width={100} >
                    <IconButton color='primary' size='small' onClick={handleSave} style={{marginLeft:20, marginTop:5}}>
                        <CheckIcon/>
                    </IconButton>
                    <IconButton color='primary' size='small' onClick={handleDeleteRowNew} style={{marginTop:5}} >
                        <DeleteIcon/>
                    </IconButton>
                </TdYounetStyled2>
                { HeadCellArr }
    ...
    const TablaSuperv = (props)=>{
        ...
        const [ periodoRutaCopy, setPeriodoRutaCopy ] = useState('');
        ...
        const handleAddRow = (info)=>{
            if(isNewRow){
                Swal.fire({
                    title:'GUARDA LA FILA NUEVA',
                    toast:true,
                    confirmButtonColor:'#0E3B5F',
                });
            }else if(isEditRow){
                Swal.fire({
                    title:'TERMINE DE EDITAR LA FILA',
                    toast:true,
                    confirmButtonColor:'#0E3B5F',
                });
            }else{ 
                const objeto = {};
                objeto.idGeneral = `rowNew_${Datos.NumMerc}_${rows.length+1}`;
                objeto.ColNew = true;
                setRows([...rows, objeto]);
                setNewRow(true);
            }
        ...
        const handleSaveRowNew = useCallback((idEl, valueEl)=>{
            const datRows = rows.map(elRow=>{
                if(elRow.idGeneral.toString() === idEl.toString()){
                    return valueEl
                }else{
                    return elRow
                }
        ...
        const handleDeleteRowNew = useCallback(()=>{
            setNewRow(false);
            const arr = [...rows];
            arr.pop();
            setRows(arr);
        ...
        const handleEditRow = useCallback((idEl, valueEl)=>{
            setEditRow(true);
            const datRows = rows.map(elRow=>{
                if(elRow.idGeneral.toString() === idEl.toString()){
                    return valueEl
        ...
        const handleDeleteRow = useCallback((idEl)=>{
            console.log(idEl);
            setEditRow(false);
            setNewRow(false);
            setRows((inf)=>inf.filter(el=>el.idGeneral !== idEl));
        },[rows]);
        ...
        const handleValidarDate = ()=>{
            const sumInicio = dataMerc.FechaInicio.split('-').map((val, index)=> index === 1 ? parseInt(val) * 30 : parseInt(val)).reduce((a,b)=> a + b);
            const sumFinal = dataMerc.FechaFinal.split('-').map((val, index)=> index === 1 ? parseInt(val) * 30 : parseInt(val)).reduce((a,b)=> a + b);
            const result = sumFinal - sumInicio;
            if(result >= 4){
                setDataMerc({...dataMerc, Periodo:`${dataMerc.FechaInicio}~${dataMerc.FechaFinal}`})
                handlePreProcesar();
            }else{
                alert('VALIDACION DE FECHA ERRÓNEA');
            }
        ...
        const handleProcesarElementos = ()=>{
            if(dataMerc.Mercaderista === ''){
                enqueueSnackbar('SELECCIONE UNA MERCADERISTA',{variant:'error'});
            }else{
                if(dataMerc.TipTraArrMerc.length > 0){
                    const headersResultStyles5 = [...headCellsColumn];
                    dataMerc.TipTraArrMerc.forEach(el=>{
                        const objeto = {...colBasic};
                        objeto.id = el.Tiptra;
                        objeto.label = el.Tiptra;
                        objeto.num = headersResultStyles5.length;
                        headersResultStyles5.push(objeto);
                    })
                    setHeadersColumn(headersResultStyles5);
                    setModeRuta(true);
                    window.localStorage.setItem('tableMSupervisora', JSON.stringify(headersResultStyles5));
                    setTimeout(()=>{
                        setSavingHeaders(false);
                    },500)
                }else{
        ...
        const handleSelectMerc = ({target})=>{ 
            setDataMerc(()=>Datos.Mercaderistas.filter(val=> val.Mercaderista === target.value)[0]);
            setSelectMercaderista(true);
        };

        const handleTipTra = (e,value)=>{ setDataMerc({...dataMerc, TipTraArrMerc:value}) };
                ...
                if(isNewRuta){
                    Swal.fire({
                        ...
                        if(respSwal.isConfirmed){
                            ...
                            objeto.Encabezados = headersColumn;
                            objeto.Data = rows;
                            objeto.idProyecto = Datos._id;
                            fetch('/Mercaderistas/Ruta',{
                                method: 'POST',
                            ...
                            .then(result =>{
                                if(result._id){
                                    ...
                                    setNewRuta(false);
                                    setDataMerc({...dataMerc, _id:result._id});
                                }else{
                                    enqueueSnackbar(result.status, {variant:'error'});
                                }
                            ...
                            enqueueSnackbar('RUTA NO GUARDADA', {variant:'error'});
                        }
                    })
                }else{
                    Swal.fire({
        ...
        const handleDataMerc = (e)=>{
            setDataMerc({...dataMerc, [e.target.id]:e.target.value})
        };
        
        const handleNewRuta = e =>{ 
            if(dataMerc.Ruta.length > 0){
                setNewRuta(e.target.checked);
            }else{
                alert('NO EXISTEN RUTAS')
            }
            
        };

        const handleSelectRuta = ({target}) =>{ setPeriodoRuta(target.value) };
        ...
        const handleProcesarRuta = () =>{
            ...
            setDataMerc({...dataMerc, _id:rutaEdit._id, Periodo:rutaEdit.Periodo, FechaInicio:rutaEdit.FechaInicio, FechaFinal:rutaEdit.FechaFinal, TipTraArrMerc:rutaEdit.TipTraArrMerc})
        ...
        const handleShowPhotos = ()=>{
            setArrPhotos(dataMerc.TipTraArrMerc);
            setTimeout(()=>{
                setOpenPhotos(true);
            },500)
        };
        ...
        const handleCopy = ({target})=>{ 
            if(isSelectMercaderista){
                if(dataMerc.Ruta.length > 0){
                    setCopy(target.checked) 
                }else{
                    alert('NO EXISTEN RUTAS A COPIAR')
                }
            }else{
                alert('DEBE SELECCIONAR UNA MERCADERISTA')
            }
            
        };

        const handleSelectRutaCopy = ({target})=>{setPeriodoRutaCopy(target.value)};

        const handleValidateCopy = ()=>{
            const sumInicio = dataMerc.FechaInicio.split('-').map((val, index)=> index === 1 ? parseInt(val) * 30 : parseInt(val)).reduce((a,b)=> a + b);
            const sumFinal = dataMerc.FechaFinal.split('-').map((val, index)=> index === 1 ? parseInt(val) * 30 : parseInt(val)).reduce((a,b)=> a + b);
            const result = sumFinal - sumInicio;
            if(result >= 4){
                const PeriodoNew = `${dataMerc.FechaInicio}~${dataMerc.FechaFinal}`;
                const arrFilPer = dataMerc.Ruta.filter(elRuta=>elRuta.Periodo === PeriodoNew);
                if(arrFilPer.length > 0){
                    alert('YA EXISTE UNA RUTA CON ESTE PERIODO');
                }else{
                    handlePreProcesarCopy();
                }
        ...
        const handlePreProcesarCopy = ()=>{
            setSavingHeaders(true);
            const arrRutaFilter = dataMerc.Ruta.filter(elRuta=>elRuta.Periodo === periodoRutaCopy)[0]
            setShowOptions(false);
            setHeadersColumn(arrRutaFilter.Encabezados);
            setRows(arrRutaFilter.Data)
            setDataMerc({...dataMerc, TipTraArrMerc:arrRutaFilter.TipTraArrMerc, Periodo:`${dataMerc.FechaInicio}~${dataMerc.FechaFinal}`})
            window.localStorage.setItem('tableMSupervisora', JSON.stringify(arrRutaFilter.Encabezados));
            setModeRuta(true);
            setTimeout(()=>{
                setSavingHeaders(false);
            },700);
        ...
        return(
            <Grid container style={{width:'100%'}}>
                    ...
                    <DialogContent>
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12} style={{height:5}}>
                                {
                                    isResetNow && <LinearProgress color='secondary' />
                                }
                            </Grid>
                            <Grid item xs={12}>
                                <TextField
                                        ...
                                        <Fragment>
                                            <Grid item xs={12}>
                                                <TextField
                                                    id='PeriodoSelect'
                                                    label='Periodo'
                                                    value={ periodoRutaCopy || ''}
                                                    onChange={handleSelectRutaCopy}
                                                    variant='outlined'
                                                    size='small'
                                                    disabled={!isSelectMercaderista}
                                                    fullWidth
                                            ...
                                            <Grid item xs={6}>
                                                <TextField
                                                    id='FechaInicio'
                                                    label='FechaInicio'
                                                    value={dataMerc.FechaInicio || '2021-08-13'}
                                            ...
                                            <Grid item xs={6}>
                                                <TextField
                                                    id='FechaFinal'
                                                    label='FechaFinal'
                                                    value={dataMerc.FechaFinal || '2021-08-13'}
                                                    onChange={handleDataMerc}
                                                    variant='outlined'
                                                    size='small'
                                                    fullWidth
                                            ...
                                            <Grid item xs={6}>
                                                <Button onClick={handleValidateCopy} color='primary' variant='contained' disabled={!isSelectMercaderista} fullWidth>
                                                    PROCESAR
                                                </Button>
                                            </Grid>
                                            <Grid item xs={6}>
                                        ...
                                        <Fragment>
                                            <Grid item xs={12} style={{padding:'5px 8px'}}>
                                                <Autocomplete
                                                    multiple
                                                    id="TIPOSTRABAJO"
                                                    options={Datos.TipTraArr}
                                                    disableCloseOnSelect
                                                    value={dataMerc.TipTraArrMerc || ''}
                                                    onChange={handleTipTra}
                                                    ...
                                                    renderInput={(params) => (
                                                        <TextField {...params} variant="outlined" label="Tipos/Trabajo" fullWidth placeholder="Tipos/Trabajo" margin='dense' />
                                                    )}
                                                />
                                            </Grid>
                                            <Grid item xs={6}>
                                            ...
                                            <Grid item xs={6}>
                                                <Button onClick={handleValidarDate} color='primary' variant='contained' disabled={!isSelectMercaderista} fullWidth>
                                                    PROCESAR
                                                </Button>
                                            </Grid>
                                            <Grid item xs={6}>
                ...
                <Grid item xl={12} lg={12} md={12} sm={12} xs={12} style={{display:'block'}}>
                    <Toolbar  >   
                        <div style={{marginRight:30, fontSize:20}}>Supervisora</div>
                        ...
                        <Tooltip title='Add Row' placement='top'>
                            <span>
                            <IconButton style={{marginRight:2}} disabled={!isFoundProject} onClick={handleAddRow} color='primary'>
                                <PlusOneIcon />
                            </IconButton>
                            </span>
                        </Tooltip>
                        <div style={{margin:'0px 0px 0px auto', display:'block'}}>
                            <Tooltip title='GUARDAR RUTA' placement='top'>
                                <span>
                                <Fab color='primary' onClick={handleSaveRuta} disabled={!isModeRuta}  aria-controls="menu-column" aria-haspopup="true"  size='small' style={{marginRight:10}}>
                                        <SaveIcon />
                                </Fab>
                                </span>
                            </Tooltip>
                            ...
                            <TableYounet>
                                ...
                                <TableBody>
                                    {stableSort(rows, getComparator(order, orderBy))
                                        ...
                                        .map((row, index) => {
                                            return (
                                                <Fragment key={row.idGeneral}>
                                                    {
                                                        row.ColNew ?
                                                        <RowNew
                                                            headCells={headersColumn} 
                                                            handleSaveRowNew={handleSaveRowNew}
                                                            handleDeleteRowNew={handleDeleteRowNew}
                                                            index={index}
                                                            row={row}
                                                            arrCadenas={arrCadenas}
                                                            onlyCadenas={onlyCadenas}
                                                        />: row.EditRowNow ?
                                                        <RowEdit
                                                            headCells={headersColumn} 
