...
import {
  ...
  import Swal from 'sweetalert2';
    ...
    const headCellsColumn = [
        { id: 'NumProf', numeric: true, disablePadding: false, label: 'Proforma', show:true, fixed:false, width:100, num:0 },
        { id: 'Cliente', numeric: false, disablePadding: false, label: 'Cliente', show:true, fixed:false, width:350, num:1 },
        { id: 'Proyecto', numeric: false, disablePadding: false, label: 'Proyecto', show:true, fixed:false, width:450, num:2 },
        { id: 'TOTAL', numeric: true, disablePadding: false, label: 'Valor', show:true, fixed:false, width:100, num:3 },
        { id: 'TiempoProyecto', numeric: true, disablePadding: false, label: 'Tiempo', show:true, fixed:false, width:100 , num:4 },
        { id: 'numpro', numeric: true, disablePadding: false, label: 'PPTO.', show:true, fixed:false, width:100, num:5  },  
        { id: 'Ejecutiva', numeric: false, disablePadding: false, label: 'Ejecutiva', show:true, fixed:false, width:250, num:6  },  
        { id: 'Status', numeric: false, disablePadding: false, label: 'Status', show:true, fixed:false, width:150, num:7  },        
        { id: 'Registro', numeric: false, disablePadding: false, label: 'Creacion', show:true, fixed:false, width:150, num:8  },
    ...
    const Roow = memo((props)=>{
        const { row, index, headCells, HandleClickRow, HandleSaveRows, handleUpdateRow } = props;
        ...
        const HeadCellArr = headCells.map((value, index2)=>{
            if(value.id === 'Status'){ 
                ...
                return(
                    <TdYounet 
                        ...
                        fixed={value.fixed}
                        color={row.Status === 'ANULADA' ? 'red' : row.Status === 'CREADA' ? 'green' : 'blue'}
                    />         
                )
                } 
            }else
            ...
            if( value.id === 'TiempoProyecto' && value.show){
                const valTiempo = `${row.TiempoProyecto} ${row.Unidad === 'MES' ? 'MES (ES)' : 'DIAS'}`
                return(
                    <TdYounet 
                        key={`${index}-${index2}-${row.idGarantia}`} 
                        width={value.width} 
        ...
        const handleCancel = ()=>{
            Swal.fire({
                title:'DESEA ELIMINAR ESTA PROFORMA?',
                ...
                if(result.isConfirmed){
                    ...
                    .then(result =>{
                        if(result.message){
                            const objeto = {...row};
                            objeto.Status = 'ANULADA';
                            handleUpdateRow(objeto);
                            Swal.fire({
                                icon:'success',
        ...
        return(
            <TrYounetStyled key={`fila-${index}-${row.idSubGarantia}`}  selected={statusRow}>
                <TableCell padding="checkbox" style={{maxWidth:100, minWidth:100}}>
                    <Grid container spacing={1} >
                        ...
                        <Grid item>
                            <IconButton size='small' color='primary' style={{marginTop:-1}} onClick={handleCancel} disabled={row.Status === 'ANULADA' ? true : false}>
                                <DeleteIcon/>
                            </IconButton>
                        </Grid>
                    </Grid>
    ...
    /*****************************************************/
        ...
        const [ headersColumn, setHeadersColumn ] = useState(()=>{
            try{
                if(window.localStorage.getItem('headersProformaMercList')){
                    const headersStorage = window.localStorage.getItem('headersProformaMercList') ;
        ...
        const [ defaultColumnWidths ] = useState([
            { columnName: 'NumProf', width: 105 },
            { columnName: 'numpro', width: 80 },
            { columnName: 'Ejecutiva', width: 160 },
            { columnName: 'Cliente', width: 100 },
            { columnName: 'Proyecto', width: 160 },
            { columnName: 'Valor', width: 140 },
        ]);
        ...
        useEffect(()=>{
            setOpen(false); 
            setHeaderWord('Mercaderista/Proforma/Listado');
            fetch('/ProformaMercs')
            .then(res=>res.json())
            .then(dataInf => {
                if(dataInf.status){
                    ...
                    if(authTokens.tipo === 'ADMINISTRADOR'){
                        ...
                        const arr = dataInf.filter(el=>el.Status !== 'ANULADA');
                        setRows(arr);
                        setBackupsRow(arr);
                    }
        ...
        /****************** MANEJADORES ****************/
            ...
            const HandleChangeSwitch = (e)=>{
                ...
                window.localStorage.setItem('headersProformaMercList', JSON.stringify(arrMap))
            };
            ...
            const handleUpdateRow = useCallback((row)=>{
                ...
                if(authTokens.tipo === 'ADMINISTRADOR'){
                    ...
                    const arr2 = arr.filter(el=>el.Status !== 'ANULADA')
                    setRows(arr2);
                    setBackupsRow(arr2);
                }
        ...
        return(
            <Grid container style={{width:'100%'}}>
                ...
                <Grid item xl={12} lg={12} md={12} sm={12} xs={12} style={{display:'block'}}>
                    <Toolbar >   
                        <div style={{marginRight:30, fontSize:20}}>Mercaderista Proforma/List</div>
    ...
    function ProformaListado(props) {
        return (
    ...
    export default ProformaListado;