...
import difDates from './helpers/difDates.js';
import PdfDinamico from './helpers/PdfDinamico';
...
const moment = require('moment'); 
    ...
    const headCellsColumn = [
        ...
        { id: 'semMesAnio', numeric: true, disablePadding: false, label: 'SEMANA DEL AÑO', show:true, fixed:false, width:150, num:1, align:'left', fontSize:'10px', bolder:false , type:'number', isHover:false, isTabla:false    },
        ...
        { id: 'opEntLogistica', numeric: true, disablePadding: false, label: 'TOTAL OP ENTR. PROD. SEMANAL', show:true, fixed:false, width:180, num:7, align:'left', fontSize:'9px', bolder:false , type:'number', hover:'opEntLogisticaHover', isHover:true, valueTabla:'opEntLogisticaTabla', isTabla:true     },
    ...
    const headersTabla_1 = [
        { title:'PPTO', mindWidth:70, align:'center', id:'numpro' },
        { title:'CLIENTE', mindWidth:200, align:'left', id:'cliente' },
        { title:'PROYECTO', mindWidth:350, align:'left', id:'proyecto' },
        { title:'APROBACION', mindWidth:80, align:'center', id:'fcreacion' },
        { title:'DIAS DIF.', mindWidth:80, align:'center', id:'diasDif' },
        { title:'INICIO', mindWidth:80, align:'center', id:'proini' },
        { title:'ACUERDO', mindWidth:80, align:'center', id:'proacu' },
        { title:'FIN', mindWidth:80, align:'center', id:'proter' },
    ];

    const headersTabla_2 = [
        { title:'TIPO TRABAJO', mindWidth:100, align:'center', id:'tiptra' },
        { title:'LOCAL', mindWidth:100, align:'center', id:'Local' },
        { title:'RESPONSABLE', mindWidth:180, align:'center', id:'logResponsable' },
        { title:'INICIO', mindWidth:80, align:'center', id:'insStart' },
        { title:'ACUERDO', mindWidth:80, align:'center', id:'insAgr' },
        { title:'FIN', mindWidth:80, align:'center', id:'insFin' },
        { title:'OBSERVACION SUP.', mindWidth:180, align:'center', id:'observacionLogistica' },
        { title:'OBSERVACION VNTS.', mindWidth:180, align:'center', id:'Observacion' },
    ];
    ...
    const TdYounet = memo(((props)=>{
        const { minWidth, width, headCells, headCell, valueInfo, fixed, color, align, bolder, fontSize, backgroundColor, valueHover, valueTabla, isTabla, title } = props
        const [ leftValue, setLeftValue ] = useState('0px');
            ...
            <Fragment>
            {
                ...
                >
                    <CelDinamica 
                        isTabla={isTabla} 
                        valueInfo={ valueInfo } 
                        isHover={headCell.isHover} 
                        valueHover={valueHover === undefined ? '': valueHover} 
                        valueTabla={valueTabla}
                        title={title}
                    />
                </TdYounetStyledFixed>
                :
    ...
    const CelDinamica = ({ valueInfo, valueHover, valueTabla, isHover, isTabla, title })=>{
        
        const [ isTable, setTable] = useState(false);
        const [ isContinuo, setContinuo ] = useState(false);
        const [ openTable, setOpenTable ] = useState(false);
        const [ getdataEl, setDataEl ] = useState([]);
        const [ isReadyPdf1, setReadyPdf1 ] = useState(false);
        const [ isReadyPdf2, setReadyPdf2 ] = useState(false);
        const [ isContinuoPdf1, setContinuoPdf1 ] = useState(false);
        const [ isContinuoPdf2, setContinuoPdf2 ] = useState(false);
        ...
        useEffect(()=>{
            if(isContinuoPdf1){
                setContinuoPdf1(false);
                setReadyPdf1(true);
            }
        },[isContinuoPdf1]); // eslint-disable-line react-hooks/exhaustive-deps

        useEffect(()=>{
            if(isContinuoPdf2){
                setContinuoPdf2(false);
                setReadyPdf2(true);
            }
        },[isContinuoPdf2]); // eslint-disable-line react-hooks/exhaustive-deps

        const handleShowTabla = ()=>{ setTable(true)};

        const handleNoShowTabla = ()=>{ setTable(false)};
    
        const handleSearchEl = async (ppto)=>{
            const info = await helperDatos({db:'Cronograma', numpro:ppto});
            if(info.length > 0){
                if(info[0].items.length > 0){
                    let enc = [];
                    info[0].encabezados.forEach(value=>{
                        enc.push(value.tiptra);
                    });
                    const arr = info[0].items.map(el=>{
                        const objeto = {...el}
                        objeto.cliente  = info[0].cliente;
                        objeto.proyecto = info[0].proyecto;
                        objeto.numpro = info[0].numpro;
                        const arrK = Object.keys(el);
                        let tip = [];
                        let valtip = 0;
                        let valReal = 0;
                        arrK.forEach(valk=>{
        ...
        const handleCreatePdf1 = ()=>{
            if(isReadyPdf1){
                setReadyPdf1(false);
                setContinuoPdf1(true);
            }else{
                setReadyPdf1(true);
            }
        };

        const handleCreatePdf2 = ()=>{
            if(isReadyPdf2){
                setReadyPdf2(false);
                setContinuoPdf2(true);
            }else{
                setReadyPdf2(true);
            }
        };

        const HandleClickPdf_1 = useCallback(()=>{
            setTimeout(()=>{
                setReadyPdf1(false);
            },2000)
        },[]);

        const HandleClickPdf_2 = useCallback(()=>{
            setTimeout(()=>{
                setReadyPdf2(false);
            },2000)
        },[]);
            ...
            <React.Fragment>
                ...
                >
                    <DialogContent>
                        <Grid container justify='space-between' spacing={0}>
                            <Grid item>
                                <Chip label={title} color='secondary' style={{fontWeight:'bolder', fontSize:'1.1rem'}}/>
                            </Grid>
                            <Grid item >
                                <Grid container spacing={1}>
                                    <Grid item>
                                        <Fab size='small' color='primary' style={{backgroundColor:'red', marginRight:10}}   onClick={handleCreatePdf1}>
                                            <PictureAsPdfIcon />
                                        </Fab>
                                    </Grid>
                                    <Grid item>
                                        {
                                            isReadyPdf1 ? <PdfDinamico data={valueTabla} headersColumn={headersTabla_1} HandleClickPdf={HandleClickPdf_1}/>
                                            : ''
                                        }
                                    </Grid>
                                </Grid>
                            ...
                            <Grid item>
                                <IconButton onClick={handleNoShowTabla} color='primary' style={{color:'red'}}>
                                    <ClearIcon/>
                                </IconButton>
                            </Grid>
                            <Grid item xs={12} style={{minHeight:490}}>
                            <TableContainer component={Paper} style={{maxHeight: 490}}>
                            <Table stickyHeader size="small"  sx={{ minWidth: 650 }} aria-label="simple table">
                                <TableHead >
                                <TableRow >
                                    { headersTabla_1.map(el=> <TableCell style={{backgroundColor:'#0E3B5F', color:"white"}} align={el.align}>{el.title}</TableCell>  )}    
                                </TableRow>
                                </TableHead>
                                <TableBody>
                                    {isTabla &&
                                    <React.Fragment>
                                        {valueTabla.map((row) => (
                                            <TableRow style={{backgroundColor:row.isProcessing ?'#F4A1C5' : '#fff', color:row.isProcessing ? '#fff' : '#000'}} hover key={row.idElement} sx={{ '&:last-child td, &:last-child th': { border: 0 } }} >
                                                <TableCell component="th" style={{fontSize:'0.8rem', fontWeight:'bolder'}} scope="row" >
                                                <Button onClick={()=>{handleSearchEl(row.numpro)}} size='small' style={{fontSize:'1rem'}}>
                                                    { row.numpro }
                                                </Button>
                                                </TableCell>
                                                ...
                                                <TableCell align="left" style={{minWidth:100, fontSize:'0.8rem', fontWeight:'bolder'}}>{row.diasDif}</TableCell>
                ...
                >
                    <DialogContent>
                        <Grid container justify='space-between' spacing={0}>
                            <Grid item>
                                <Chip label={`PPTO:${getdataEl[0]?.numpro} - CLIENTE:${getdataEl[0]?.cliente}`} color='secondary' style={{fontWeight:'bolder', fontSize:'1.1rem'}}/>
                            </Grid>
                            <Grid item >
                                <Grid container spacing={1}>
                                    <Grid item>
                                        <Fab size='small' color='primary' style={{backgroundColor:'red', marginRight:10}}   onClick={handleCreatePdf2}>
                                            <PictureAsPdfIcon />
                                        </Fab>
                                    </Grid>
                                    <Grid item>
                                    {
                                        isReadyPdf2 ? <PdfDinamico data={getdataEl} headersColumn={headersTabla_2} HandleClickPdf={HandleClickPdf_2}/>
                                        : ''
                                    }
                                    </Grid>
                                </Grid>
                            ...
                            <Grid item>
                                <IconButton onClick={handleCloseTable} color='primary' style={{color:'red'}}>
                                    <ClearIcon/>
                                </IconButton>
                            </Grid>
                            <Grid item xs={12}>
                                <TableContainer component={Paper} style={{maxHeight: 490}}>
                                    <Table stickyHeader size="small"  sx={{ minWidth: 650 }} aria-label="simple table">
                                        <TableHead >
                                        <TableRow >   
                                            { headersTabla_2.map(el=> <TableCell style={{backgroundColor:'#0E3B5F', color:"white"}} align={el.align}>{el.title}</TableCell> ) }                 
                                        </TableRow>
                                        </TableHead>
                                        <TableBody>
                                            {isTabla &&
                                            <React.Fragment>
                                                {getdataEl.map((row, index) => (
                                                    <TableRow hover key={`subEl_${index}_${new Date()}`} sx={{ '&:last-child td, &:last-child th': { border: 0 } }} >
                                                        <TableCell align="left" style={{fontSize:'0.8rem', fontWeight:'bolder'}} scope="row"> {row.tiptra}</TableCell>
                                                        <TableCell align="left" style={{fontSize:'0.8rem', fontWeight:'bolder'}} scope="row"> {row.Local}</TableCell>
                                                        <TableCell align="left" style={{fontSize:'0.7rem', fontWeight:'bolder'}}>{row.logResponsable}</TableCell>
                                                        <TableCell align="left" style={{minWidth:100, fontSize:'0.8rem', fontWeight:'bolder'}}>{row.insStart}</TableCell>
                                                        <TableCell align="left" style={{minWidth:100, fontSize:'0.8rem', fontWeight:'bolder'}}>{row.insAgr}</TableCell>
                                                        <TableCell align="left" style={{minWidth:100, fontSize:'0.8rem', fontWeight:'bolder'}}>{row.insFin}</TableCell>
                                                        <TableCell align="left" style={{fontSize:'0.7rem', fontWeight:'bolder'}}>{row.observacionLogistica}</TableCell>
                                                        <TableCell align="left" style={{fontSize:'0.7rem', fontWeight:'bolder'}}>{row.Observacion}</TableCell>
                                                    </TableRow>
                                                ))}
                                            </React.Fragment>}
                ...
                <Tooltip title={valueHover} >
                    <Button onClick={handleShowTabla} size='small' style={{fontSize:'1rem', color:'black'}}>
                        { valueInfo }
                    </Button>
                </Tooltip>:
                <Button onClick={handleShowTabla} size='small'  style={{fontSize:'1rem', color:'black'}} disabled={!isTabla}>
                    { valueInfo }
                </Button>
                }
    ...
    const TablaSuperv = (props)=>{
                ...
                for(const x in resultOp){
                    ...
                    if(resultOp[x].ot)
                        if(resultOp[x].ot !== null || resultOp[x].ot !== undefined || resultOp[x].ot !== '' || resultOp[x].ot !== ' '){
                            ...
                            objeto3.isProcessing = isProcessing > 0 ? true : valorIni === 0 || valorAcu === 0 || valorTer === 0 ? true :false;
                    ...
                    if(getOpciones.isSemAnio){
                        for(let i = 0; i < getOpciones.semAnioArr.length; i++){
                            ...
                            for(let p = 0; p< arr_nico.length; p++){
                                ...
                                const fechaAcuerdo1 = moment(arr_nico[p].fechaAcu.toString().split('T')[0]);
                                const diasDif1 = difDates(fechaAprobacion1, fechaAcuerdo1);
                                if(parseInt( diasDif1 ) > 3){ 
                                    arr_Atraso.push(arr_nico[p].numpro)
                                    const objeto_ = {...arr_nico[p]};
                                    objeto_.diasDif = diasDif1;
                                    arr_AtrasoTabla.push(objeto_)
                                }
                            }
                            const objeto={};
                            objeto.semMes = '';
                        ...
                        const numCronPen1 = [];
                        const numCronPen1Tabla = [];
                        const numCronPen2 = [];
                        const numCronPen2Tabla = [];
                        const numCronPen3 = [];
                        const numCronPen3Tabla = [];
                        const numCronPen4 = [];
                        const numCronPen4Tabla = [];
                        const numCronPen5 = [];
                        const numCronPen5Tabla = [];
                        const numCronPen6 = [];
                        const numCronPen6Tabla = [];
                            ...
                            switch(arrfilAnualAcuerdo1_[u].semMesAcu){
                                case 1:
                                    ...
                                    if(parseInt( diasDif1 ) > 3){
                                        ...
                                        const objetoDif_1 = {...arrfilAnualAcuerdo1_[u]};
                                        objetoDif_1.diasDif = diasDif1;
                                        numAtrFecSem1Tabla.push(objetoDif_1);
                                    }
                                    break;
                                case 2:      
                                    ...
                                    const diasDif2 = difDates(fechaAprobacion2, fechaAcuerdo2);
                                    if(parseInt( diasDif2 ) > 3){
                                        numAtrFecSem2.push(arrfilAnualAcuerdo1_[u].numpro);   
                                        const objetoDif_2 = {...arrfilAnualAcuerdo1_[u]};
                                        objetoDif_2.diasDif = diasDif2;     
                                        numAtrFecSem2Tabla.push(objetoDif_2);           
                                    }             
                                    break;
                                case 3: 
                                    ...
                                    const diasDif3 = difDates(fechaAprobacion3, fechaAcuerdo3);
                                    if(parseInt( diasDif3 ) > 3) {
                                        numAtrFecSem3.push(arrfilAnualAcuerdo1_[u].numpro); 
                                        const objetoDif_3 = {...arrfilAnualAcuerdo1_[u]};
                                        objetoDif_3.diasDif = diasDif3;     
                                        numAtrFecSem3Tabla.push(objetoDif_3);     
                                    }
                                    break;
                                case 4:
                                    ...
                                    const diasDif4 = difDates(fechaAprobacion4, fechaAcuerdo4);
                                    if(parseInt( diasDif4 ) > 3){
                                        numAtrFecSem4.push(arrfilAnualAcuerdo1_[u].numpro);
                                        const objetoDif_4 = {...arrfilAnualAcuerdo1_[u]};
                                        objetoDif_4.diasDif = diasDif4;     
                                        numAtrFecSem4Tabla.push(objetoDif_4);
                                    }
                                    break;
                                case 5:
                                    ...
                                    const diasDif5 = difDates(fechaAprobacion5, fechaAcuerdo5);
                                    if(parseInt( diasDif5 ) > 3){
                                        numAtrFecSem5.push(arrfilAnualAcuerdo1_[u].numpro);
                                        const objetoDif_5 = {...arrfilAnualAcuerdo1_[u]};
                                        objetoDif_5.diasDif = diasDif5;     
                                        numAtrFecSem5Tabla.push(objetoDif_5);
                                    }
                                    break;
                                case 6:
                                    ...
                                    const diasDif6 = difDates(fechaAprobacion6, fechaAcuerdo6);
                                    if(parseInt( diasDif6 ) > 3){
                                        numAtrFecSem6.push(arrfilAnualAcuerdo1_[u].numpro);
                                        const objetoDif_6 = {...arrfilAnualAcuerdo1_[u]};
                                        objetoDif_6.diasDif = diasDif6;     
                                        numAtrFecSem6Tabla.push(objetoDif_6);
                                    }
                                    break;
                                default:
                                    console.log('NO APLICA')
                        ...
                        for(const u in arrMensual1){
                            switch(arrMensual1[u].semMes){
                                case 1:
                                    ...
                                    if(arrMensual1[u].proini === 0 && arrMensual1[u].proacu === 0 && arrMensual1[u].proter === 0){
                                        numCronPen1.push(arrMensual1[u].numpro);
                                        numCronPen1Tabla.push(arrMensual1[u]);
                                    }
                                    break;
                                case 2:
                                    ...
                                    if(arrMensual1[u].proini === 0 && arrMensual1[u].proacu === 0 && arrMensual1[u].proter === 0){
                                        numCronPen2.push(arrMensual1[u].numpro);
                                        numCronPen2Tabla.push(arrMensual1[u]);
                                    }                                                    
                                    break;
                                case 3:
                                    ...
                                    if(arrMensual1[u].proini === 0 && arrMensual1[u].proacu === 0 && arrMensual1[u].proter === 0){
                                        numCronPen3.push(arrMensual1[u].numpro);
                                        numCronPen3Tabla.push(arrMensual1[u]);
                                    }
                                    break;
                                case 4:
                                    ...
                                    if(arrMensual1[u].proini === 0 && arrMensual1[u].proacu === 0 && arrMensual1[u].proter === 0){
                                        numCronPen4.push(arrMensual1[u].numpro);
                                        numCronPen4Tabla.push(arrMensual1[u]);
                                    }
                                    break;
                                case 5:
                                    ...
                                    if(arrMensual1[u].proini === 0 && arrMensual1[u].proacu === 0 && arrMensual1[u].proter === 0){
                                        numCronPen5.push(arrMensual1[u].numpro);
                                        numCronPen5Tabla.push(arrMensual1[u]);
                                    }
                                    break;
                                case 6:
                                    ...
                                    if(arrMensual1[u].proini === 0 && arrMensual1[u].proacu === 0 && arrMensual1[u].proter === 0){
                                        numCronPen6.push(arrMensual1[u].numpro);
                                        numCronPen6Tabla.push(arrMensual1[u]);
                                    }
                                    break;
                                default:
                                    console.log('NO APLICA')
                        ...
                        for(let i =1; i <= 6; i++){
                            ...
                            switch(i){
                                case 1:
                                    ...
                                    objeto.cronpen = numCronPen1.length;
                                    objeto.cronpenHover = numCronPen1.toString();
                                    objeto.cronpenTabla = numCronPen1Tabla;
                                    arrTot.push(objeto);
                                    break;
                                case 2:
                                    ...
                                    objeto.cronpen = numCronPen2.length;
                                    objeto.cronpenHover = numCronPen2.toString();
                                    objeto.cronpenTabla = numCronPen2Tabla;
                                    arrTot.push(objeto);
                                    break;
                                case 3:
                                    ...
                                    objeto.cronpen = numCronPen3.length;
                                    objeto.cronpenHover = numCronPen3.toString();
                                    objeto.cronpenTabla = numCronPen3Tabla;
                                    arrTot.push(objeto);
                                    break;
                                case 4:
                                    ...
                                    objeto.cronpen = numCronPen4.length;
                                    objeto.cronpenHover = numCronPen4.toString();
                                    objeto.cronpenTabla = numCronPen4Tabla;
                                    arrTot.push(objeto);
                                    break;
                                case 5:
                                    ...
                                    objeto.cronpen = numCronPen5.length;
                                    objeto.cronpenHover = numCronPen5.toString();
                                    objeto.cronpenTabla = numCronPen5Tabla;
                                    arrTot.push(objeto);
                                    break;
                                case 6:
                                    ...
                                    objeto.cronpen = numCronPen6.length;
                                    objeto.cronpenHover = numCronPen6.toString();
                                    objeto.cronpenTabla = numCronPen6Tabla;
                                    arrTot.push(objeto);
                                    break;
                                default:
                                    console.log('NO APLICA')
        ...
        const handleSemAnio = ({target})=>{ setOpciones({ ...getOpciones, isSemAnio:target.checked, Mes:'' })};
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12}>
                                <Grid component="label" container alignItems="center" spacing={1} justify='center'>
                                    ...
                                    <Grid item>
                                        <Switch checked={getOpciones.isSemAnio || false} onChange={handleSemAnio} name="semAnioChecked" />
                                    </Grid>
                            ...
                            </Grid>
                            {
                                ...
                                <Grid item xs={12} style={{minHeight:90}}>
                                   <Autocomplete
                                    ...
                                    //getOptionLabel={(option) => option.semAnio}
                                    renderOption={(option, { selected }) => (
                                        <React.Fragment  key={`${option}_${new Date()}`}>
                                            <Checkbox
                                                icon={icon}
                                                checkedIcon={checkedIcon}
                                                style={{ marginRight: 8 }}
