...
const moment = require('moment'); 
    ...
    const headersTabla_2 = [
        ...
        { title:'RESPONSABLE', mindWidth:180, align:'center', id:'proResponsable' },
        { title:'INICIO', mindWidth:80, align:'center', id:'proini' },
        { title:'ACUERDO', mindWidth:80, align:'center', id:'proacu' },
        { title:'FIN', mindWidth:80, align:'center', id:'proter' },
        { title:'OBSERVACION', mindWidth:180, align:'center', id:'observacionProduccion' },
    ];
    ...
    const CelDinamica = ({ valueInfo, valueHover, valueTabla, isHover, isTabla, title })=>{
        ...
        const handleSearchEl = async (ppto)=>{
            const info = await helperDatos({db:'Matriz', numpro:ppto});
            if(info.length > 0){
                if(info[0].formulario.length > 0){
                    const arr = info[0].formulario.map(el=>{
                        const objeto = {...el}
            ...
            <React.Fragment>
                ...
                >
                    <DialogContent>
                        <Grid container justify='space-between' spacing={0}>
                            <Grid item>
                                <Chip label={`PPTO: ${getdataEl[0]?.numpro} - ${getdataEl[0]?.proyecto}`} color='secondary' style={{fontWeight:'bolder', fontSize:'1.1rem'}}/>
                            </Grid>
                            <Grid item >
                                <Grid container spacing={1}>
                                    <Grid item>
                                            ...
                                            <React.Fragment>
                                                {getdataEl.map((row, index) => (
                                                    <TableRow hover key={`subEl_${index}_${new Date()}`} sx={{ '&:last-child td, &:last-child th': { border: 0 } }} >
                                                        <TableCell component="th" style={{fontSize:'0.8rem', fontWeight:'bolder'}} scope="row"> {row.tiptra}</TableCell>
                                                        <TableCell align="left" style={{fontSize:'0.7rem', fontWeight:'bolder'}}>{row.proResponsable}</TableCell>
                                                        <TableCell align="left" style={{minWidth:100, fontSize:'0.8rem', fontWeight:'bolder'}}>{row.proini}</TableCell>
                                                        <TableCell align="left" style={{minWidth:100, fontSize:'0.8rem', fontWeight:'bolder'}}>{row.proacu}</TableCell>
                                                        <TableCell align="left" style={{minWidth:100, fontSize:'0.8rem', fontWeight:'bolder'}}>{row.proter}</TableCell>
                                                        <TableCell align="left" style={{fontSize:'0.7rem', fontWeight:'bolder'}}>{row.observacionProduccion}</TableCell>
                                                    </TableRow>
                                                ))}
                                            </React.Fragment>}
    ...
    const TablaSuperv = (props)=>{
                        ...
                        if(resultOp[x].ot !== null || resultOp[x].ot !== undefined || resultOp[x].ot !== '' || resultOp[x].ot !== ' '){
                            ...
                            if(arrFiltrado3.length > 0){
                                for(const u in arrFiltrado3){
                                    ...
                                    if(arrFiltrado3[u].proacu){
                                        if(arrFiltrado3[u].proacu !== 'N/A' || arrFiltrado3[u].proacu !== '' || arrFiltrado3[u].proacu !==  null || arrFiltrado3[u].proacu !== undefined){
                                            ...
                                            if(arrFiltrado3[u].proter){
                                                if(arrFiltrado3[u].proter !== 'N/A' || arrFiltrado3[u].proter !== '' || arrFiltrado3[u].proter !==  null || arrFiltrado3[u].proter !== undefined){
                                    ...
                                    if(arrFiltrado3[u].fecproacu){
                                        if(arrFiltrado3[u].fecproacu !== 'N/A' || arrFiltrado3[u].fecproacu !== '' || arrFiltrado3[u].fecproacu !==  null || arrFiltrado3[u].fecproacu !== undefined){
                                            
                                            let valAcu_ = new Date(arrFiltrado3[u].fecproacu );
                                            let acuBack_ = new Date(fecacu_);
                                            if(valAcu_.getTime() > acuBack_.getTime()){ 
                                                fecacu_ = valAcu_;
                                                valorAcu_ = arrFiltrado3[u].fecproacu;
                                            }
        ...
        const handleSelectSemAnio = (e, value)=>{ 
            //console.log(value)
            setOpciones({ ...getOpciones, semAnioArr:value }) 
        };
        ...
        return(
            <Grid container style={{width:'100%'}}>
                ...
                <Grid item xl={12} lg={12} md={12} sm={12} xs={12} style={{display:'block'}}>
                    <Toolbar  >   
                        <div style={{marginRight:30, fontSize:20}}>Produccion</div>
