...
import Viewer from 'react-viewer';
    ...
    const headCellsColumn = [
        { id: 'cliente', numeric: false, disablePadding: false, label: 'CLIENTE', show:true, fixed:false, width:200, num:0, align:'left', fontSize:'10px', bolder:false , type:'text'    },
    ...
    const TablaSuperv = (props)=>{
        ...
        useEffect(()=>{
            const handleUseEffect = async ()=>{
                setOpen(false); 
                setHeaderWord('Reporteria/Historico');
                setCharging(false);
                const getFUsers = await fetch('/UsersList')
                const getDUsers = await getFUsers.json();
                //const arrFilter = getDUsers.map(el=>el.coordinador);
                //const filterCoor = arrFilter.filter((el,index)=> arrFilter.indexOf(el) === index);
                //setCoordinadoras(filterCoor);
        ...
        const HandleChangeSwitch = (e)=>{
            ...
            window.localStorage.setItem('ReporteriaHeadersHistorico', JSON.stringify(arrMap))
        };
        ...
        const HandleProcessHeaders = useCallback((dataHeadersStyle)=>{
            ...
            window.localStorage.setItem('ReporteriaHeadersHistorico', JSON.stringify(headersResultStyles))
            setSavingHeaders(true);
        ...
        const handleDeleteColumn = ()=>{
            ...
            window.localStorage.setItem('ReporteriaHeadersHistorico', JSON.stringify(arrSetOrder))
            setSavingHeaders(true);
            setTimeout(()=>{
                setSavingHeaders(false);
            },2000);
        ...
        const handleReset = ()=>{
            ...
            window.localStorage.setItem('ReporteriaHeadersHistorico', JSON.stringify(headCellsColumn));
            setRows([]);
            setTimeout(()=>{
                setSavingHeaders(false);
                setResetNow(false);
        ...
        const handleProcesarElementos = ()=>{
            ...
            switch(getOpciones.tipoFiltro){
                case 'EJECUTIVA':
                    const arrEje3 = dataArr.filter(el=>el.ejecutiva === getOpciones.Ejecutiva); //ARR ELEMENTOS APROBADOS FILTRADOS POR EJECUTIVA
                    switch(getOpciones.Tabla){
                        case 'COMPARATIVO':
                            const arrfilM = arrEje3.filter(el=> getOpciones.MesesComparar.includes(el.mesCreacion))
                            const arrClientes_0 = arrfilM.map(el=>el.cliente);
                            const arrClientes = arrClientes_0.filter((el,index)=> arrClientes_0.indexOf(el) === index);
                            ...
                            const arrdata_0 = arrClientes.map(el=>{
                                const objeto = {};
                                objeto.Cliente = el;
                                for(const p in getOpciones.MesesComparar){
                                    let mesComp = getOpciones.MesesComparar[p];
                                    let anioIni = getOpciones.AnioInicio;
                                    const arrFilCli_0 = arrInicio.filter(el2=>el2.cliente.toString().trim() == el.toString().trim());
                                ...
                                for(const p in getOpciones.MesesComparar){
                                    ...
                                    const arrFilCli_2 = arrFinal.filter(el2=>el2.cliente.toString().trim() == el.toString().trim());
                            ...
                            const arrHeaders = [];
                            const objeto_cli = { };
                                objeto_cli.id       = 'Cliente';
                                objeto_cli.label    = 'CLIENTE';
                            ...
                            window.localStorage.setItem('ReporteriaHeadersHistorico', JSON.stringify(arrHeaders));
                            setTimeout(()=>{
                                setSavingHeaders(false);
                            },1000);
                            break;
                        default:
                            switch(getOpciones.Tipo){
                                case 'ANUAL':
                                    ...
                                    const arrClientes_0Anual = arrfilAnual.map(el=>el.cliente); 
                                    const arrClientesAnual = arrClientes_0Anual.filter((el,index)=> arrClientes_0Anual.indexOf(el) === index); //ARR SOLO CLIENTES
                                    const arrdataAnual_0 = arrClientesAnual.map(el=>{
                                        const objeto = {};
                                        objeto.Cliente = el;
                                        for(const p in Meses){
                                            let mesComp = Meses[p];
                                            const arrFilCli_2 = arrfilAnual.filter(el2=>el2.cliente.toString().trim() == el.toString().trim());
                                            const arrFilCli_3 = arrFilCli_2.filter(el2=>el2.mesCreacion.toString().trim() == mesComp.toString().trim());
                                    ...
                                    const objeto_cliA = { };
                                        objeto_cliA.id       = 'Cliente';
                                        objeto_cliA.label    = 'CLIENTE';
                                    ...
                                    if(getOpciones.Porcentaje){
                                        ...
                                        const arrdataAnualPorcentaje = arrdataAnual.map(el=>{
                                            ...
                                            objeto.Porcentaje = parseFloat(divT).toFixed(2);
                                            return objeto;
                                        })
                                        ...
                                        window.localStorage.setItem('ReporteriaHeadersHistorico', JSON.stringify(arrHeadersAnual));
                                        setTimeout(()=>{
                                            setSavingHeaders(false);
                                        },1000);
                                    }else{
                                ...
                                default:
                                    ...
                                    const arrClientes_0Anual1 = arrfilAnual1.map(el=>el.cliente); 
                                    const arrClientesAnual1 = arrClientes_0Anual1.filter((el,index)=> arrClientes_0Anual1.indexOf(el) === index); //ARR SOLO CLIENTES
                                    const arrdataAnual1_0 = arrClientesAnual1.map(el=>{
                                        const objeto = {};
                                        objeto.Cliente = el;
                                        for(const p in getOpciones.MesesComparar){
                                            let mesComp = getOpciones.MesesComparar[p];
                                            const arrFilCli_2 = arrfilAnual1.filter(el2=>el2.cliente.toString().trim() == el.toString().trim());
                                            const arrFilCli_3 = arrFilCli_2.filter(el2=>el2.mesCreacion.toString().trim() == mesComp.toString().trim());
                                            const arrFilCli   = arrFilCli_3.map(el2=> el2.subFac);
                                            const valFilCliIni = arrFilCli.reduce((a,b)=> parseFloat(a) + parseFloat(b) ,0);
                                            objeto[`${mesComp}`] = parseFloat(valFilCliIni).toFixed(2);
                                    ...
                                    const objeto_cliA1 = { };
                                        objeto_cliA1.id       = 'Cliente';
                                        objeto_cliA1.label    = 'CLIENTE';
                                    ...
                                    const objeto_total1 = { };
                                        objeto_total1.id       = 'Total';
                                        objeto_total1.label    = 'TOTAL';
                                    ...
                                    arrHeadersAnual1.push(objeto_total1)
                                    ...
                                    window.localStorage.setItem('ReporteriaHeadersHistorico', JSON.stringify(arrHeadersAnual1));
                                    setTimeout(()=>{
                                        setSavingHeaders(false);
                                    },1000);
                                    break;
                    ...
                    break;
                case 'COORDINADORA':
                    const arrCoor3 = dataArr.filter(el=>el.coordinadora === getOpciones.Coordinadora);
                    switch(getOpciones.Tabla){
                        case 'COMPARATIVO':
                        ...
                        default:
                            switch(getOpciones.Tipo){
                                ...
                                default:
                                    ...
                                    const objeto_total2 = { };
                                        objeto_total2.id       = 'Total';
                                        objeto_total2.label    = 'TOTAL';
                                    ...
                                    arrHeadersAnual1.push(objeto_total2)
                                    setRows(arrdataAnual1);
                                    setBackupsRow(arrdataAnual1);
                                    setHeadersColumn(arrHeadersAnual1);
                                    getOpciones.Seleccionable && setSelectionArr(arrdataAnual1);
        ...
        const handleOpcionesEjecutiva = (e, value)=>{
            if(value === '' || value === null){
                setOpciones({...getOpciones, Ejecutiva:''});
            }else{ 
                setOpciones({...getOpciones, Ejecutiva:value});
            }
        };

        const handleOpcionesCoordinadora = (e, value)=>{
            if(value === '' || value === null){
                setOpciones({...getOpciones, Coordinadora:''});
            }else{ 
                setOpciones({...getOpciones, Coordinadora:value});
            }
        };
        ...
        return(
            <Grid container style={{width:'100%'}}>
                            ...
                            </Grid>
                            {
                                ...
                                <Grid item xs={12}>
                                    <Autocomplete
                                        ...
                                        margin='dense'
                                        style={{marginTop:-8}}
                                        renderInput={params => <TextField {...params}  variant="outlined" margin='dense' label='COORDINADORA'/>}
                                ...
                                <Grid item xs={12}>
                                    <Autocomplete
                                        ...
                                        margin='dense'
                                        style={{marginTop:-8}}
                                        renderInput={params => <TextField {...params}  variant="outlined" margin='dense' label='EJECUTIVA'/>}
                ...
                <Grid item xl={12} lg={12} md={12} sm={12} xs={12} style={{display:'block'}}>
                    <Toolbar  >   
                        <div style={{marginRight:30, fontSize:20}}>Histórico</div>
