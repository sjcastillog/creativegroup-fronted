...
import {
  ...
  import Swal from 'sweetalert2';
    ...
    const headCellsColumn = [
        { id: 'changeNA', numeric: false, disablePadding: false, label: 'Action', show:true, fixed:false, width:120, num:1, type:'text', align:'left', fontSize:'10px', bolder:false},
        { id: 'ejecutiva', numeric: false, disablePadding: false, label: 'Ejecutiva', show:true, fixed:false, width:120, num:2, type:'text', fontSize:'10px'},
        { id: 'numpro', numeric: true, disablePadding: false, label: 'Numpro', show:true, fixed:false, width:100, num:3, type:'number', align:'left', fontSize:'10px', bolder:false},
        { id: 'cliente', numeric: false, disablePadding: false, label: 'Cliente', show:true, fixed:false, width:250, num:4, type:'text', align:'left', fontSize:'10px', bolder:false },
        { id: 'Proyecto', numeric: false, disablePadding: false, label: 'Proyecto', show:true, fixed:false, width:350, num:5, type:'text', align:'left', fontSize:'10px', bolder:false },
        { id: 'proResponsable', numeric: false, disablePadding: false, label: 'Responsable', show:true, fixed:false, width:280, num:6, type:'text', align:'left', fontSize:'10px', bolder:false },
        { id: 'creacion', numeric: false, disablePadding: false, label: 'Creacion', show:true, fixed:false, width:120, num:7, type:'date', align:'left', fontSize:'10px', bolder:false },
        { id: 'tiptra', numeric: false, disablePadding: false, label: 'Tipo Trabajo', show:true, fixed:false, width:200, num:8, type:'text', align:'left', fontSize:'10px', bolder:false },      
        { id: 'cantreq', numeric: true, disablePadding: false, label: 'Cantidad', show:true, fixed:false, width:100, num:9, type:'number', align:'left', fontSize:'10px', bolder:false }, 
        { id: 'fecreqcli', numeric: false, disablePadding: false, label: 'Fecha Req.', show:true, fixed:false, width:120, num:10, type:'date', align:'left', fontSize:'10px', bolder:false },  
        { id: 'proini', numeric: false, disablePadding: false, label: 'Inicio', show:true, fixed:false, width:200, num:11, type:'date', align:'left', fontSize:'10px', bolder:false },
        { id: 'proacu', numeric: false, disablePadding: false, label: 'Acuerdo', show:true, fixed:false, width:200, num:12, type:'date', align:'left', fontSize:'10px', bolder:false },
        { id: 'proter', numeric: false, disablePadding: false, label: 'Fin', show:true, fixed:false, width:200, num:13, type:'date', align:'left', fontSize:'10px', bolder:false },
        { id: 'observacionProduccion', numeric: false, disablePadding: false, label: 'Observacion', show:true, fixed:false, width:250, num:14, type:'text', align:'left', fontSize:'10px', bolder:false },
        { id: 'StatusProduccion', numeric: false, disablePadding: false, label: 'Status', show:true, fixed:false, width:100, num:15, type:'text', align:'left', fontSize:'10px', bolder:false },
    ];
    ...
    const TdYounet = memo(((props)=>{
        const { minWidth, width, headCells, headCell, valueInfo, fotos, fotosFirst, fixed, color, changeNA, proResponsable, fechas, observacionProduccion, HandleSaveRows  } = props
        const [ leftValue, setLeftValue ] = useState('0px');
        ...
        return (
            <TableHead style={{position:'sticky', top:0, zIndex:2}}>
                ...
                <TheadYounetStyled width={60}>
                    <CheckBoxIcon style={{marginLeft:7}}/>
                </TheadYounetStyled>
                {headCells.map( headCell =>  
                    headCell.show && ( 
                        headCell.fixed 
                        ? <TheadYounetFixed  headCells={headCells} key={headCell.id} widthDefault={headCell.width} createSortHandler={createSortHandler} order={order} orderBy={orderBy} headCell={headCell} classes={classes} />
                        : <TheadYounet  headCells={headCells} key={headCell.id} widthDefault={headCell.width} createSortHandler={createSortHandler} order={order} orderBy={orderBy} headCell={headCell} classes={classes} />
                ))}
                </TableRow>
            </TableHead>
        );
    ...
    /*****************************************************/
        ...
        const [ headersColumn, setHeadersColumn ] = useState(()=>{
            try{
                if(window.localStorage.getItem('headersSupProduccion')){
                    const headersStorage = window.localStorage.getItem('headersSupProduccion') ;
        ...
        useEffect(()=>{
            setOpen(false); 
            setHeaderWord('Supervisor/Produccion');
            (async ()=>{
                      ...
                      if(subRow.tiptra !== 'INSTALACIONES / ENTREGA / MONTAJE'){
                        if(subRow.tiptra !== 'DESMONTAJE / RETIRO'){
                          ...
                          if(found.creacion){
                            ...
                            subFechC = `${fechaC.getFullYear()}-${fechaC.getMonth() + 1}-${fechaC.getDate()}`;
                          }else if(found.fecer){
                          ...
                          if(subRow.fecreqcli){
                            ...
                            subFechR = `${fechaR.getFullYear()}-${fechaR.getMonth() + 1}-${fechaR.getDate()}`;
                          }else if(row.fecreqcli){
                            let fechaR = new Date(row.fecreqcli);
                            subFechR = `${fechaR.getFullYear()}-${fechaR.getMonth() + 1}-${fechaR.getDate()}`;
                          }
                              Objeto.Proyecto = row.proyecto;
                              Objeto.proResponsable = subRow.proResponsable;
                              Objeto.numpro = subRow.numpro;
                              Objeto.ejecutiva = row.ejecutiva;
                              Objeto.cliente = row.cliente;
                              Objeto.idMatriz = row._id;
                              Objeto.id = subRow._id ? subRow._id : `${row._id}-sub-${subIndex}`;
                              Objeto.idGeneral = subRow._id ? subRow._id : `${row._id}-sub-${subIndex}`;
                              Objeto.idElemento = subRow._id ? subRow._id : `${row._id}-sub-${subIndex}`;
                              ...
                              Objeto.StatusProduccion = subRow.StatusProduccion;
                              Objeto.cantreq = subRow.cantreq;
                              Objeto.fecreqcli = subFechR;
                              Objeto.observacionProduccion =  subRow.observacionProduccion;
                              Objeto.creacion =  subFechC === '' ? '2019-01-01' : subFechC;
                              Objeto.anioCreacion = subFechC === '' ? subFechC.split('-')[0] : '2019';
                              Objeto._id = subRow._id ? subRow._id : `${row._id}-sub-${subIndex}`;
                              arr.push(Objeto);
                        }
                      }
                    }
        ...
        /****************** MANEJADORES ****************/
            ...
            const HandleChangeSwitch = (e)=>{
                ...
                window.localStorage.setItem('headersSupProduccion', JSON.stringify(arrMap))
            };
            ...
            const HandleProcessHeaders = useCallback((dataHeadersStyle)=>{
                ...
                window.localStorage.setItem('headersSupProduccion', JSON.stringify(headersResultStyles))
                setSavingHeaders(true);
        ...
        return(
            <Grid container style={{width:'100%'}}>
                ...
                <Grid item xl={12} lg={12} md={12} sm={12} xs={12} style={{display:'block'}}>
                    <Toolbar >   
                        <div style={{marginRight:30, fontSize:20}}>Supervisor Produccion</div>
    ...
    const Roow = memo((props)=>{
        ...
        const HeadCellArr = headCells.map((value, index2)=>{
            ...
            if( value.id === 'proini' || value.id === 'proacu' || value.id === 'proter'){
                if(value.show){
                return(
                    <TdYounet 
                        key={`${index}-${index2}-${row.idGarantia}`} 
            ...
            if( value.id === 'observacionProduccion' && value.show){
                return(
                    <TdYounet 
                        key={`${index}-${index2}-${row.idGarantia}`} 
                        width={value.width} 
                        ...
                        data={row}
                        element={true}
                        observacionProduccion={true}
                        HandleSaveRows={HandleSaveRows}
                    /> 
                );
            }else
            if( value.id === 'proResponsable' && value.show){
                return(
                    <TdYounet 
                        key={`${index}-${index2}-${row.idGarantia}`} 
                        width={value.width} 
        ...
        return(
            <TrYounetStyled key={`fila-${index}-${row.idElemento}`}  selected={statusRow}>
    ...
    const CellRow = memo((props)=>{
        ...
        const HandleSave = ()=>{
            const objeto = {...data};
            objeto.observacionProduccion = infoCell;
    ...
    const CellRowChangeNA = ({ data, statusRow, HandleSaveRows })=>{
        ...
        return(
            <Fragment>
                ...
                <IconButton size='small' onClick={HandleOpenAnulado} disabled={data.Status === 'Anulado' ? true : false} style={{marginTop:-8}}>
    ...
    const CellRowDate = (props)=>{
        ...
        const HandleSave = ()=>{
            ...
            objeto.StatusProduccion = headCell.id === 'proter' ? 'Concluido' : 'En Proceso';
            HandleSaveRows(objeto);
            fetch('/FechasProduccionSupervisor',{
                method: 'PUT',
                body: JSON.stringify({objetoFronted:objeto}),
        ...
        const HandlePreSave = ()=>{
            ...
            if(headCell.id === 'proacu'){
                const dataInicio = new Date (data.proini);
        ...
        if(editMode){
            ...
            return(
                <Button 
                    ...
                    onClick={HandleEditMode}
                    color="primary"
                    style={{textAlign:'center', textDecoration:'underline', padding:0, margin:0, marginTop:-10, height:30 }}
                >
                    {info || '__/__/____'}
                </Button>
            );
    ...
    const CellRowResponsable = (props)=>{
        ...
        const HandleSave = ()=>{
            const objeto = {...data};
            objeto.proResponsable = infoCell;
            objeto.ColumnaEl = headCell.id;
            HandleSaveRows(objeto);
            fetch('/FechasProduccionSupervisor',{
                method: 'PUT',
        ...
        if(editMode){
            return(
                ...
                <Grid container style={{width:headCell.width, marginTop:-12}} spacing={1} >
            ...
            return(
                <Button 
                    ...
                    onClick={HandleEditMode}
                    color="primary"
                    style={{textAlign:'center', textDecoration:'underline', marginTop:-10, padding:0, height:30 }}
                >
                    {info || 'Click'}
                </Button>
            );
